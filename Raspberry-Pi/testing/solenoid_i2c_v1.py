...
try:
    ...
    for char in text.lower():
        if char.isalpha():
            ...
            print(f"\nDisplaying: '{char}' (Braille Index: {index})")

            # Set solenoids according to braille pattern
            for i in range(6):
                channel = solenoid_channels[i]
                dot_state = braille[index][i]
                set_solenoid_state(channel, dot_state)
